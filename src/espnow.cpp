...
QueueHandle_t recv_from_EspNow_queue;
...
// Forward message to Queue recv_from_EspNow_queue
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  if (xQueueSend(recv_from_EspNow_queue, (void *)incomingData, 0) != pdTRUE) {
...
bool espNowSend(const String &message) {
  if (message.length() <= ESP_BUFFER_SIZE) {
    ...
    Serial.println("String Message too long");
    return false;
  }
  return false;
}
...
bool espNowSend(const JsonDocument &doc) {
  ...
  if (jsonMessage.length() <= ESP_BUFFER_SIZE) {
    ...
    Serial.println("JSON Message too long");
    return false;
  }
  return false;
}
...
bool initEspNow() {
  ...
  recv_from_EspNow_queue = xQueueCreate(ESPNOW_QUEUE_SIZE, ESP_BUFFER_SIZE);
  if (recv_from_EspNow_queue == NULL) {
    Serial.println("Create Queue failed");
    return false;
  }
